...
\begin{center} \fbox{
    \parbox{10cm}{
        ...
        \begin{equation} \label{eq:sumrule-sigma}
            \large
            \int_{s_0}^\infty \frac{\sigma_2(s) - \sigma_0(s)}{s - m} \dd{s} = 0
        \end{equation}
    }
} \end{center}

\paragraph{Charmonium in the final state}
When we are looking at radiative transitions between charmonium bound states, the superconvergent relation~\cref{eq:sumrule-sigma} simplifies a lot. In the transition we actually have a process $2 \rightarrow 1$, which is naturally can be expressed in terms of decay width $1 \rightarrow 2$. Relation between cross-section and decay width is easy to derive in center of mass frame (see the full derivation in Appendix~\cref{eq:app:crsc-dw}):
